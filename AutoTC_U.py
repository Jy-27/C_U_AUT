...
class analysis:
    def __init__(self, websocket_data, ohlcv_days, ohlcv_minute):
        self.websocket_data = websocket_data
        self.ohlcv_days = ohlcv_days
        self.ohlcv_minute = ohlcv_minute
    ...
    async def case_1_UL(self):
        ...
        if n < 3:
            ...
            close_price_max = max(self.ohlcv_minute[self.ohlcv_minute.index >= timedelta]['close'])
    ...
    async def case_2_UL(self):
        ...
        if self.realtime_data[2]['open'] is not None:
            ...
            # delta_str = timedelta.strftime('%Y-%m-%d %H:%M:%S')
            close_price_max = max(self.ohlcv_minute[self.ohlcv_minute.index >= timedelta]['close']) < self.realtime_data[0]['close']
            ...
            trade_price_max_ = max(self.ohlcv_minute[self.ohlcv_minute.index >= timedelta]['close']) < self.realtime_data[0]['close']
...
class get_ohlcv:
    def __init__(self):
        self.days = {}
        self.minute = {}
        self.tickers = pu.get_tickers('KRW')
    async def update(self):
        print('ohlcv updating...')
        for ticker in self.tickers:
            # print(f'현재 줄 번호 >>> {sys._getframe().f_lineno}')
            self.days[ticker] = pu.get_ohlcv(ticker=ticker, interval='days', count=365)
            # print(f'현재 줄 번호 >>> {sys._getframe().f_lineno}')
            self.minute[ticker] = pu.get_ohlcv(ticker=ticker, interval='minute5', count=288)
            await asyncio.sleep(0)
        time_ = datetime.datetime.now()
        print(f'ohlcv update complete- {time_.strftime('%Y-%m-%d %H:%M:%S')}')
    async def get_data(self, interval, ticker):
        if interval == 'days' and ticker in self.days:
            return self.days[ticker]
        elif interval == 'minute' and ticker in self.days:
            return self.minute[ticker]
        ...
        if tickers_target is not None:
            ...
            for ticker in tickers_target:
                try:
                    ...
                    ohlcv_days = await updater_ohlcv.get_data(interval='days', ticker=ticker)
                    ohlcv_minute = await updater_ohlcv.get_data(interval='minute', ticker=ticker)
                    analysys_ = analysis(websocket_data=get_data, ohlcv_days=ohlcv_days, ohlcv_minute=ohlcv_minute)
...
async def main():
    ...
    ohlcv_ = get_ohlcv()
    updater_2_task = asyncio.create_task(updater_2(instance=ohlcv_, check_instance=tickers_))

    q_ = asyncio.Queue()
    websocket_task = asyncio.create_task(websocket(queue=q_, updater=tickers_))
    monitoring_task = asyncio.create_task(monitoring(queue=q_, updater_ticker=tickers_, updater_ohlcv=ohlcv_))
